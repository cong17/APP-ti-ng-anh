...
class Course extends Component {
	...
	loadCourse = () => {
		...
		axios.get(url + 'courses-api/' + this.state.courseId + '/').then(res => {
			...
			if (words.length === 0) {
				course.words = [{ word: 'Chủ đề này đang trống', description: '' }];
			} else {
				course.words = words;
			}
					...
					{this.state.added && (
						<div className={styles.ProgressDiv}>
							<div className={styles.WordsLearned}>
								{this.state.wordsLearned} / {course.totalWords} Từ đã học
							</div>
							<div className={styles.ProgressBar}>
								<div style={progressWidth} className={styles.Progress} />
							</div>
							{progress === 100 && <div className={styles.CourseCompleted}>Khóa học đã hoàn thành!</div>}
